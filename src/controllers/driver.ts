...
const getDrivers = async (req: Request, res: Response) => {
    ...
    try {
        ...
        return res.json(new EntityListResponse(drivers, totalDrivers, page, totalPages));
    } catch (error) {
        return res.status(500).json({ message: 'No se obtuvo la lista de choferes' });
    }
};
...
const getDriver = async (req: Request, res: Response) => {
    try {
        ...
        return res.status(500).json({ message: 'No se obtuvo el chofer con ese id' });
    }
};
